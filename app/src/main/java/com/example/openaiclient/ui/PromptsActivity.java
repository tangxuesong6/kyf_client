...
import com.example.openaiclient.PromptsAdapter;
...
public class PromptsActivity extends AppCompatActivity {
    ...
    private EditText edtPrompts;
    private MyProgressDialog myProgressDialog;
    private List<PromptsBean> promptList;
    ...
    private void initPrompts() {
        if (!ConfigSharedPreferences.getInstance().hasPrompts(this)) {
            ConfigSharedPreferences.getInstance().savePrompts(this, "[\n" +
                    "  {\n" +
                    "    \"act\": \"担任雅思写作考官\",\n" +
    ...
    private void initListener() {
        promptsAdapter.setOnItemClickListener(new PromptsAdapter.OnItemClickListener() {
            @Override
        ...
        update.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                String content = edtPrompts.getText().toString();
                if (TextUtils.isEmpty(content)) {
                    Toast.makeText(PromptsActivity.this, "请输入内容", Toast.LENGTH_SHORT).show();
                    return;
                }
                myProgressDialog.show();
                NetUtil.getInstance().get(content, new ResponseCallback<byte[]>() {
                    @Override
                    public void success(byte[] resp) {
                        String result = new String(resp);
                        LogUtil.d("result", result);
                        Type listType = new TypeToken<List<PromptsBean>>() {
                        }.getType();
                        Gson gson = new Gson();
                        List<PromptsBean> promptList = gson.fromJson(result, listType);
                        if (promptList.size() > 1) {
                            PromptsActivity.this.promptList.clear();
                            PromptsActivity.this.promptList.addAll(promptList);
                            ConfigSharedPreferences.getInstance().savePrompts(PromptsActivity.this, result);
                            MainThread.run(new Runnable() {
                                @Override
                                public void run() {
                                    Toast.makeText(PromptsActivity.this, "更新成功", Toast.LENGTH_SHORT).show();
                    ...
                    public void fail(String error) {
                        ...
                        MainThread.run(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(PromptsActivity.this, "更新失败", Toast.LENGTH_SHORT).show();
    ...
    private void initView() {
        edtPrompts = findViewById(R.id.input_prompt);
        if (TextUtils.isEmpty(edtPrompts.getText())) {
            edtPrompts.setText("https://raw.githubusercontent.com/PlexPt/awesome-chatgpt-prompts-zh/main/prompts-zh.json");
        }
        update = findViewById(R.id.tv_update_prompt);
        myProgressDialog = new MyProgressDialog(this, R.style.dialogTransparent, R.layout.lottie_dialog);
        myProgressDialog.setCancelable(false);
        String prompts = ConfigSharedPreferences.getInstance().getPrompts(this);
        Type listType = new TypeToken<List<PromptsBean>>() {
        }.getType();
        Gson gson = new Gson();
        promptList = gson.fromJson(prompts, listType);
        promptsAdapter = new PromptsAdapter(promptList);
        promptsRecyclerView = findViewById(R.id.rv_prompts);
